...
| `grader.py` | `Grader` service singleton. Builds the officer prompt, calls Ollama `/api/chat` directly with `think:false`, parses strict JSON, and falls back to deterministic string matching. Never raises. |
